...
def is_snippet(abbr):
	...
	@return bool
	"""
	return abbr in zen_settings['html']['snippets']

def parse_into_tree(abbr):
	"""
	...
	def token_expander(operator, tag_name, id_attr, class_name, multiplier):
		...
		current = is_snippet(tag_name) and Snippet(tag_name, multiplier) or Tag(tag_name, multiplier)
...
def find_abbr_in_line(line, index = 0):
	...
	cur_index = index - 1
	while cur_index >= 0:
		if not is_allowed_char(line[cur_index]):
...
class Tag(object):
	def __init__(self, name, count = 1):
		...
		"""
		name = name.lower()
		self.name = Tag.get_real_name(name)
		self.count = count
		self.children = []
		self.attributes = []
		
		if name in zen_settings['html']['default_attributes']:
			for k,v in zen_settings['html']['default_attributes'][name].items():
	...
	def get_real_name(name):
		...
		"""
		real_name = name
		aliases = zen_settings['html']['aliases']
	...
	def is_empty(self):
		...
		@return: bool
		"""
		return self.name in zen_settings['html']['empty_elements']
	...
	def is_inline(self):
		...
		@return: bool
		"""
		return self.name in zen_settings['html']['inline_elements']
	...
	def is_block(self):
		...
		@return: bool
		"""
		return self.name in zen_settings['html']['block_elements']
...
class Snippet(Tag):
	...
	def to_string(self, format = False, indent = False):
		data = zen_settings['html']['snippets'][self.name]
		begin = ''
		end = ''
		content = ''
		child_padding = ''
